...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_path",
        type=str,
        default="./data/pretraining_data/unconstrained_language",#"./data/pretraining_data/constrained_language",
    ...
    parser.add_argument(
        "--tokenizer_path",
        type=str,
        default="./data/trained_tokenizers/tokenizer_files_unconstrained_language/roberta-base_31000",#"./data/trained_tokenizers/tokenizer_files_constrained_language/roberta-base_19000"
...
def vary_configuration():
    ...
    for feature in features_to_vary:        
        for feature_val in features_to_vary[feature]:
            ...
            save_dir = os.path.join(f"./../results", date_)
            eval_file = os.path.join(save_dir, f"experiment_results_eval_{timestamp_}_{feature}.csv")
            test_file = os.path.join(save_dir, f"experiment_results_test_{timestamp_}_{feature}.csv")
            ...
            # save results
            test_results.to_csv(test_file)
            eval_results.to_csv(eval_file)
            
            # update map (wandb run name -> model configuration)
            try:
...
def train_a_set_of_configurations():
    ...
    for exp in all_experiments:        
        ...
        # path to save files at
        save_dir = os.path.join(f"./../results", date_)
        eval_file = os.path.join(save_dir, f"experiment_results_eval_{timestamp_}.csv")
        test_file = os.path.join(save_dir, f"experiment_results_test_{timestamp_}.csv")
